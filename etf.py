...
def etf(portfolio : pd.DataFrame) -> pd.Series:
    ...
    for t in range(1, etf.shape[0]):
        ...
        for i in portfolio.columns:
            ...
            holdings_t_minus_1 = holdings(portfolio, hodls, i, t - 1, etf[t - 1])
...
def universe_to_returns(universe : str, start_date: Optional[str] = None, end_date : Optional[str] = None) -> pd.Series:
    ...
    historical_prices = get_historical_prices(universe,
...
def etfs_and_components_to_portfolio(etfs : pd.DataFrame, xs : Dict[str, float], weights : Dict[str, float]) -> pd.DataFrame:
    ...
    for key, val in etfs.items():
        ...
        arrays = [['Date', 'Open', 'High', 'Low', 'Close', 'Adj Close', 'Volume', 'Weight'], prices.index.levels[1]]
        df = prices.reindex(pd.MultiIndex.from_product(arrays, names=['Field', 'Date']))
        df.columns = [key]
        df[key].loc["Weight"] = np.repeat(weights[key], df[key].loc["Weight"].index.shape[0])
        components.append(df)

    for key, val in xs.items():
        arrays = [['Date', 'Open', 'High', 'Low', 'Close', 'Adj Close', 'Volume'], pd.DatetimeIndex(val['Date'])]
        index = pd.MultiIndex.from_product(arrays, names=['Field', 'Date'])
        df = pd.DataFrame(val.unstack().values, index=index)
        df.columns = [key]
        arrays = [['Date', 'Open', 'High', 'Low', 'Close', 'Adj Close', 'Volume', 'Weight'], pd.DatetimeIndex(val['Date'])]
